...
def index(request):
    ...
    try:
        ...
        if meta_section:
            meta_contents = {c.content_key: c.value for c in meta_section.
                                                                contents.all()}
            context['meta_title'] = meta_contents.get('title',
                                                      'Мишки Тедди'
                                                      ' ручной работы')
            ...
            context['meta_description'] = ('Авторские мишки Тедди ручной работы'
                                           ' для вашей коллекции')
            context['meta_keywords'] = ('коллекционные мишки тедди, авторский'
                                        ' мишка тедди')

        # 2. Hero-секция (главный баннер)
        hero_section = PageSection.objects.filter(section_key='hero').first()
        if hero_section:
            hero_contents = {c.content_key: c.value for c in hero_section.
                                                                contents.all()}
            context['hero_title'] = hero_contents.get('titleHero', 'Уникальные'
                                                                   ' мишки '
                                                                   'Тедди с'
                                                                   ' душой')
        ...
        logger.debug(f"Показывать статические карточки: "
                     f"{context['has_static_shop_items']}")

        # 4. Секция "О мастере"
        about_section = PageSection.objects.filter(section_key='about').first()
        if about_section:
            about_contents = {c.content_key: c.value for c in about_section.
                                                                contents.all()}
            context['about_title1'] = about_contents.get('aboutTitleBlock1',
                                                         'История мастера')
            context['about_description1'] = (about_contents
                                             .get('aboutDescriptionBlock1',
                                                   'Добро пожаловать в мой мир'
                                                   ' handmade-творчества!'
                                                   ' Уже более 5 лет я создаю ' 
                                                   'уникальных мишек Тедди, '
                                                   'каждый из которых '
                                                   'становится особенным '
                                                   'другом для своего'
                                                   ' владельца. Моя философия'
            ...
            context['about_item1'] = about_contents.get('aboutSlot1Block2',
                                                        '5+ лет опыта в '
                                                        'создании мишек Тедди')
            context['about_item2'] = about_contents.get('aboutSlot2Block2',
                                                        '200+ довольных '
                                                        'клиентов')
            context['about_item3'] = about_contents.get('aboutSlot3Block2',
            ...
            context['about_description1'] = ('Добро пожаловать в мой мир '
                                             'handmade-творчества! Уже более 5'
                                             ' лет я создаю уникальных мишек'
                                             ' Тедди, каждый из которых '
                                             'становится особенным другом для'
                                             ' своего владельца. Моя философия'
                                             ' — это внимание к деталям,'
        ...
        if contacts_section:
            contacts_contents = \
              {c.content_key: c.value for c in contacts_section.contents.all()}

            # Город и адрес
            context['contacts_city'] = contacts_contents.get('contactsCity',
                                                             'Санкт-Петербург')
            context['contacts_address'] = (contacts_contents
                                           .get('contactsAddress',
                                                'ул. Среднерогатская'))

            # Координаты для карты
            context['contacts_latitude'] = (contacts_contents
                                            .get('contactsPoints_latitude',
                                                 '59.819987'))
            context['contacts_longitude'] = (contacts_contents
                                             .get('contactsPoints_longitude',
                                                  '30.337649'))

            # Контактная информация
            context['contacts_phone'] = contacts_contents.get('contactsPhone',
                                                          '+7 (911) 999-99-99')
            context['contacts_email'] = contacts_contents.get('contactsEmail',
                                                          'example@example.ru')

            # Социальные сети
            context['contacts_vk'] = contacts_contents.get('contactsVK',
                                                   'https://vk.com/id39146412')
            context['contacts_whatsapp'] = (contacts_contents
                                            .get('contactsWhatsApp',
                                                 'https://wa.me/79111292655'))
            context['contacts_telegram'] = (contacts_contents
                                            .get('contactsTelegramm',
                                                 'https://t.me/Elen0Fil'))

            # Логирование координат для отладки
            logger.debug(f"Координаты из базы данных: Широта="
                         f"{context['contacts_latitude']}, " +
            ...
            logger.warning("Секция 'contacts' не найдена в базе данных,"
                           " используются значения по умолчанию")

        # Общая информация о загрузке
        logger.info(f"Главная страница успешно загружена. Товаров:"
                    f" {shop_items.count()}, " +
                    f"Координаты: {context['contacts_latitude']},"
                    f" {context['contacts_longitude']}")

    except Exception as e:
        # Детальное логирование ошибки без прерывания работы сайта
        logger.error(f"КРИТИЧЕСКАЯ ОШИБКА при загрузке данных главной"
                     f" страницы: {str(e)}", exc_info=True, stack_info=True)
